...
class Edge():
    ...
    def SetColor(self, window, color):
        pygame.draw.lines(window, color, True, [self.node1.center, self.node2.center], EdgeThickness)
        myfont = pygame.font.SysFont('Comic Sans MS', 15)
        textsurface = myfont.render(str(self.weight), False, (5, 5, 5))
        window.blit(textsurface, (
        (self.node1.center[0] + self.node2.center[0]) / 2, (self.node1.center[1] + self.node2.center[1]) / 2))
...
class VisualizationClass():
    def __init__(self):
        ...
        cost(window, white, str("(K190232) & (K191288)"))
        pygame.display.update()  #
        time.sleep(2)

        # window = pygame.display.set_mode([1500, 700])
        # cost(window, white, str("input10.txt"))
        # pygame.display.update()  #
        # time.sleep(2)
        ...
        while True:
            ...
            for index, node in enumerate(inputnodes):
                tempnode = Node(yellow, node, 20, 1, index)
            ...
            if not called:
                ...
                time.sleep(3)
                called = True
                #Boruvka(nodes,5,pygame.display.set_mode([1500,700]))
                #pygame.display.update()
                #time.sleep(3)
...
def Prims(graph, source, window):
    ...
    while edges != []:
        ...
        if find(parent, e.node1.index) != find(parent, e.node2.index):
            ...
            edges.remove(e)
    ans = "PRIMS : Mst c : " + str(c)
    for tempnode in graph:
        tempnode.Draw(window)
    graph[source].SetColor(window, blue)
    cost(window, white, ans)
    print(ans)
    pygame.display.update()
...
def FloydWarshall(graph, window):
    ...
    cost(window, white, str("FloydWarshall total cost : ") + str(total_cost))
